...
/**
 * In the HoconMapper we convert a class into an ObjectNode or load
 * an ObjectNode into a class using Typesafe Config.
 *
 * @author EinNik
 * @since 4.0.0-SNAPSHOT
 */
open class HoconMapper : BaseHoconMapper() {

    @Throws(Exception::class)
    fun saveToMap(clazz: Class<Any>): ObjectNode {
        val node = objectMapper.createObjectNode()

        if (!clazz.superclass.equals(HoconConfig::class.java)) {
        ...
        for (field: Field in clazz.declaredFields) {
            ...
            if (value != null) {
                setNestedValue(node, path, objectMapper.valueToTree(value))
            }
        }
    ...
    private fun setNestedValue(node: ObjectNode, path: String, value: JsonNode) {
        ...
        for (i in 0 until keys.size - 1) {
            ...
            if (!current.has(key) || !current.get(key).isObject) {
                current.set<JsonNode>(key, objectMapper.createObjectNode())
            }
